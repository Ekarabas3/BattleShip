...
public class Board extends Parent{
    ...
    public Board(int[] carrierArray, int[] battleshipArray, int[] cruiserArray,
            ...
            for (int x = 0; x < 10; x++) {
                ...
                if (carrierOrientation == 'h') {
                        ...
                        for (int i = 0; i < carrierArray.length; i++) {
                            if(isLegal(carrierOrientation, x, y, carrierArray.length)){
                                if (carrierArray[i] == y) {
                                    Cell c = new Cell(x,y,this, true);
                                }
                            }
                            else{
                                System.out.println("You cannot place the ship here");
                                break;
                            }
                        }
        ...
        getChildren().add(rows);
    */}
    public boolean isLegal(char ori, int x, int y, int length){
        if(ori == 'h'){
                if(x + length >= 10){
                return false;
            }
